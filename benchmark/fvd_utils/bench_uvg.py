...
# 定义视频压缩和性能计算函数
def compress_and_evaluate(input_folder, output_folder, codec, qp_range, pix_fmt, width_in, height_in, framerate_in ,bitdepth = 8):
    ...
    for qp in qp_range:
        ...
        command = f"ffmpeg -framerate {framerate_in} -video_size {width_in}x{height_in} -i {input_folder}/city_128x128_30Hz_8bit_yuv420p8.yuv -c:v {codec} -preset veryfast -s 128x128 -crf {qp} -pix_fmt {pix_fmt} -tune zerolatency {compressed_file} -y"
...
def main():
    ...
    # Common options.
    parser.add_argument("--dataset", type=str, default="/media/myworkstation/文档/Dataset/UVG/test/", help="dataset path")

    parser.add_argument(
        "--output_path",
        type=str,
        default="/media/myworkstation/文档/output_uvg_bench/",
    ...
    compress_and_evaluate(input_yuv, output_folder_264, 'libx264', qp_range, "yuv420p",args.width, args.height, args.framerate)

    # Compress and evaluate using H.265 (commented command)
    compress_and_evaluate(input_yuv, output_folder_265, 'libx265', qp_range, "yuv420p",args.width, args.height, args.framerate)
